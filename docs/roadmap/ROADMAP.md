...
`WP-R2-01`–`WP-R2-04`、`WP-R3-01` 与 `WP-R3-02` 已经 `Done`，R2 里程碑关闭，R3 已交付学期生命周期、Course/Meeting 有效日期范围，以及跨重启身份稳定的重复 Meeting occurrence/segment。当前唯一 `Ready` 的主链工作包是 [`WP-R3-03`](./BACKLOG.md#r3--可用课表)：补齐时间、TermZone、冲突和 TBA 语义。工作包生命周期仍只以 [Backlog 注册表与证据台账](./BACKLOG.md#6-证据台账) 为准；本段仅同步当前指针。
