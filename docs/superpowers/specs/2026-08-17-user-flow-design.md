...
- 课程级教学日期范围及课节范围继承。
- 首页的今日完成数、今日待完成数和学期日期进度。首页页眉提升的三个数字于 2026-08-31 重排为逾期任务、今日待完成与下一节，今日已完成移入「今日任务」的完成度（见 UI 规格 §8.1）。
- 多日假期在周日历中的单一跨日长条。
...
| UF-A-06 | 在本周、周网格和选中日详情中理解课节、任务、假期与 TBA | 核心 MVP-A |
...
首页页眉右侧显示学期进度胶囊，以及三个行动数字：

- 逾期任务。
- 今日待完成。
- 下一节的时刻；学期尚未开始时降级为学期开始日期。

今日已完成仍然是正式统计口径，改由「今日任务」卡的完成度呈现，不再占用页眉位置。三个数字与胶囊的确切事实来源和降级阶梯见 UI 规格 §8.1。
...
- 选中日详情在该假期覆盖的每一天显示同一条带起止日期的范围事项。
...
2. 宽幅标题：“今天先做什么”。
3. 右上：学期进度胶囊与三个行动数字（逾期任务、今日待完成、下一节）。
4. 四列三行网格：左侧今日时间轴日脊；中间「现在」（环）、「本周课时」（七日柱图）与「今日任务」；右侧唯一深色卡「下一步 + 本周截止」。
5. 底行：当前学期「课程」accordion 与按课程统计的「学期任务」完成度。
6. 逾期在页眉与任务列表可见，本周时间冲突在「本周课时」卡底部，TBA 事项计入「学期任务」卡，明细仍在任务页。
...
**用户目标**：在本周列表、周网格和选中日详情中理解所有计划事项及假期影响。

**主流程**：

1. 用户从首页进入本周或周日历。
2. 本周列表按日期和时间展示课节、任务、截止事项及命名假期。
3. 周网格的时间格展示有精确时间的课节和任务。
4. 纯日期任务进入全天层；该周没有纯日期任务和可见假期时，全天层不出现。
5. 日期或时间未知的任务进入待确定区域。
6. 地点为 TBA 的课节仍有明确时间，继续保留在时间格中。
7. 连续假期在全天层显示为一个跨日长条。
8. 用户点击假期长条打开同一个日期范围对象。
9. 用户点击列头或用方向键选中一天，网格下方的详情只展示该日的课节、任务与假期，不再整周平铺。
10. 用户切换周时，选中日复位到新一周内的今天；今天不在该周时复位到该周的周一。
11. 用户点击页眉的时间冲突提示时，网格滚动到第一组冲突所在的列并选中该日。
12. 应用从周网格和选中日详情提供返回事项详情、创建事项和编辑例外的入口。
13. 应用打开时，根据同一组实例显示本周列表、倒计时和临近提示；应用关闭后不承诺系统通知。

**选中日规则**：

- 选中日是 Renderer 的视图状态，不是查询参数；它只决定详情显示哪一天。
- 「今天」标记始终归属真实当前日期，不随浏览的周移动，也不随选中日移动。
- 时间冲突只来自统一计划投影已给出的警告，应用不在视图层重新判定重叠。
...
**完成状态**：本周列表、周网格和选中日详情展示同一组实例与状态，且假期范围可追溯到单一对象。
