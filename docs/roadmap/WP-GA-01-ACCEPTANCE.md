...
| G6 产品环境 | 部分通过，尚未全门通过 | clean 0e55715 双平台 typecheck/默认并行全套/package/smoke 已齐；Windows 758 pass / 0 fail / 1 权限 skip，macOS 752 pass / 0 fail / 7 Windows 专属 skip。真人已由用户确认完成，制品关联待补录；实际禁网与完整失败旅程 artifact 仍缺，见第 4、5 节 |
| G7 性能基线 | Windows 已批准预算通过；Mac 普通端点通过，全门待补证 | clean 0e55715 的 Windows 普通端点、匹配对照与严格后台预算、运行时/迁移/恢复资源观察已归档；Mac 普通 packaged 前台端点 PASS，仍缺 Mac 匹配对照/严格后台预算及适用 ADR 观测。历史失败保留原身份，见第 5 节 |
...
| Windows 制品 | 同一 clean source、锁文件经 `pnpm package --platform win32 --arch x64` 在 Mac 交叉打包 exit 0；文件识别为 PE32+ x86-64。main/preload/workspace 三入口静态嵌入相同 development identity；该 Mac 交叉制品未在 Windows 执行；同源 Windows 原生 package/smoke 与性能已另见上节，不混用制品来源 |
...
该 Mac 轮次没有取得 `_scratch` 中后台 v2、ADR、迁移和恢复观察器，因此只完成上述可复用 CLI 的本机证据。
合并时已在 Windows 工作区定位最终同源观察器交接包；尚未取得这些工具在 Mac 的实际结果。
真人验收已由本轮用户确认完成，制品关联待补录；实际禁网旅程与完整失败 artifact 仍缺；macOS 已完成 UI 身份仍为 b39124d，不重开原矩阵。
合并 Windows 归档后，G7 仍缺 macOS 严格后台与合格对照、适用 ADR-02/03/04/07/08 剩余运行时观测；
...
G1 的验收口径已获用户确认，默认并行测试、恢复授权及打包查询预算阻断均已修复。
同一 clean 0e55715 的双平台自动化和普通端点已通过；Windows 匹配对照/严格后台预算与适用运行时补测已归档。
剩余动作集中为 G6 的用户已确认真人验收的制品关联补录、实际禁网旅程及完整失败 artifact；
G7 的 macOS 匹配无备份对照/严格后台预算及 ADR-02/03/04/07/08 剩余运行时、迁移、备份、恢复与资源观测。
Mac 已完成的 UI 验收继续保留，不要求重做；Windows 真人验收也沿用用户确认，仅补其证据关联。
