...
- [ ] **Step 2: 写最小包清单与 pnpm 策略**
  ...
  pnpm 11 的 `.npmrc` 只用于 registry/auth；`nodeLinker` 与 `saveExact` 必须由 workspace 配置读取。`blockExoticSubdeps: false` 会为整个 workspace 关闭 exotic transitive dependency 拦截；当前 `pnpm-lock.yaml` 中唯一此类条目是 Forge `7.11.2` 通过固定 commit 的官方 `@electron/node-gyp` tarball。任何未来依赖或锁文件变更都必须重新审查 exotic sources。
...
- [ ] **Step 3: 写 TypeScript 配置**
  ...
  `tsconfig.test.json` extends 主配置，覆盖 `module: Node16`、`moduleResolution: Node16`、`noEmit: false`、`rootDir: .`、`outDir: .test-dist`、`declaration: false`、`sourceMap: false`，include `src/shared/**/*.ts`、`src/main/runtime-paths.ts`、`tests/**/*.ts`。`package.json` 不含 `type: module`，所以 `.ts` 测试仍输出 CommonJS，同时使用 TypeScript 7 支持的 Node 解析模式。
...
- [ ] **Step 5: 写 Forge/Vite 配置**
  ...
  Workspace Node/Worker entry 在 Task 7 创建真实 `src/workspace.ts` 时加入；其 `main` target 只选择官方 Node/Worker 构建模板。

  renderer 只包含 `{ name: 'main_window', config: 'vite.renderer.config.ts' }`。

  两个 Vite 配置都调用同一个 `readDevelopmentBuildId()`，以 `define` 注入 `__COURSEFLOW_APP_BUILD_ID__`。Node 配置 externalize `electron`，renderer `root` 为 `src/renderer`，二者均关闭 sourcemap、关闭 minify，renderer `base` 为 `./`。renderer 的 `build.outDir` 必须为项目根绝对路径 `.vite/renderer/main_window`：`root` 保持源目录身份，而 Forge packager 只收集项目根 `/.vite`，不能落到 `src/renderer/.vite`。不要加入 React plugin；Vite 对 TSX 的内置转换已足够，R1 不需要 Fast Refresh plugin。
...
- [ ] **Step 7: 关闭并提交**
  ...
  此时尚无 `src/main.ts`，所以不把 `pnpm typecheck` 当作成功条件；运行 `pnpm exec tsc --ignoreConfig --skipLibCheck --noEmit --target ES2023 --module ESNext --moduleResolution Bundler --types node build/read-development-build-id.ts forge.config.ts vite.node.config.ts vite.renderer.config.ts` 验证配置本身。将 `WP-R1-01` 设为 `Done`，`WP-R1-02` 设为 `Ready`，更新证据台账，运行 `git diff --check` 后提交：

  ```powershell
  git add .gitignore pnpm-workspace.yaml package.json pnpm-lock.yaml tsconfig.json tsconfig.test.json forge.config.ts vite.node.config.ts vite.renderer.config.ts build/read-development-build-id.ts src/vite-env.d.ts docs/roadmap/BACKLOG.md
...
#### 审查修正：Task 6 packaged shell 安全与可加载性复验

首次完成记录只能证明进程和标题，不能证明 packaged renderer 真实加载；独立审查因此将 `WP-R1-02` 重新打开。上游 `98bcf7d` 已将 renderer 输出布局修正为 Forge 会复制进 app 的 `.vite/renderer/main_window`，但 Task 6 必须重新运行 package 与内容验收，不能把修正推断为已验证。

1. 先扩展 `tests/architecture/renderer-boundary.test.ts` 并运行 `pnpm test`。守卫须递归扫描所有 renderer `.ts/.tsx`，以 TypeScript AST 覆盖 side-effect、dynamic import、`require`、`electron/*`、`node:*`、`fs/path/sqlite` 子路径以及 runtime capability/平台路径；BrowserWindow 检查必须绑定到真实 `new BrowserWindow({...})` 的 `webPreferences`，包括 `webSecurity: true`。测试同时要求限制性 CSP、`will-navigate` 阻止、`setWindowOpenHandler` 返回 `deny`，以及 session 的 permission check/request 均拒绝。此时预期因现有壳缺 CSP 和这些 handlers 而 RED；普通 `pnpm test` 不读取或依赖 `out/`。
2. 仅在 RED 后以最小实现补充 CSP，并把 navigation、新窗口和权限拒绝绑定到该窗口的 `webContents`/`session`；不得开放外部 URL、shell、网络 capability，也不得新增依赖。`index.html` 中唯一额外的 `<link rel="stylesheet" href="./styles.css">` 是加载本任务已经计划的 CSS 所需的最小标签，不移动 CSS，也不添加声明文件。
3. GREEN 后进行受控 nested renderer side-effect-import mutation，证明递归 AST 守卫确实失败，再清理并重跑 GREEN。
4. 运行 `pnpm test`、`pnpm typecheck`、`pnpm package`。仅在 package 后，使用已安装的 asar 工具列出当前 `out/.../resources/app.asar`，断言 `.vite/build/main.js`、`.vite/renderer/main_window/index.html`、renderer CSS 和 renderer JS 同时存在，并与 Main 的 `loadFile(path.join(__dirname, '../renderer', MAIN_WINDOW_VITE_NAME, 'index.html'))`、renderer name 和 outDir 一致。该 asar 检查是 post-package 门，不替代日常 unit test。
5. 不启动 dev server，直接启动该 package 的 Windows EXE，记录精确 PID，通过窗口 handle 截图保存到 ignored SDD 目录并实际查看：`CourseFlow`、`本地优先课程工作区`、`桌面运行时已启动` 均可见，且不是错误或空白页。只以 `CloseMainWindow()` 和该精确 PID 关闭。记录 asar 清单、截图、退出、平台限定和审查重开/修复事实后，才可将 `WP-R1-02` 设为 `Done`、`WP-R1-03` 设为 `Ready`。
...
- [ ] **Step 2: 实现 Workspace utility 入口**
  ...
  在同一 VitePlugin `build` 数组加入 `{ entry: 'src/workspace.ts', config: 'vite.node.config.ts', target: 'main' }`；`main` 只选择官方 Node/Worker 构建模板，entry 保留 Workspace 产物身份。
...
- [ ] **Step 7: 验证进程契约并提交**
  ...
  ```powershell
  git add src tests forge.config.ts docs/roadmap/BACKLOG.md
