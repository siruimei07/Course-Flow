import { INTL_ZONE_RULES, findMeetingTimeOverlap } from '../meeting-time';
...
import type { MeetingOccurrenceProjection } from '../workspace-course-contract/types';
import { HolidayRangeProjection } from '../workspace-holiday-contract';
import { isDateInWindow, isExactDataArray, isPlainObject } from './guards';
import { TaskOccurrenceProjection, TaskSize } from '../workspace-task-contract';
import { TermProjection, localDateInTermZone } from '../workspace-term-contract';
...
export const MINIMUM_LOCAL_DATE_COORDINATE = Date.parse('0001-01-01T00:00:00.000Z');

export const MAXIMUM_LOCAL_DATE_COORDINATE = Date.parse('9999-12-31T00:00:00.000Z');

export const MAXIMUM_CANONICAL_INSTANT = '9999-12-31T23:59:59.999Z';
 ...
 */
export function taskDeadlineBoundary(
    occurrence: TaskOccurrenceProjection,
 ...
 */
export function taskOccurrenceIdentityKey(occurrence: TaskOccurrenceProjection): string {
 ...
 * @return {string} Stable series-and-anchor ordering key.
 */
export function meetingOccurrenceIdentityKey(occurrence: MeetingOccurrenceProjection): string {
 ...
 */
export function compareCanonicalText(first: string, second: string): number {
 ...
 */
export function planOccurrenceIdentityKey(item: PlanTaskProjection | PlanMeetingProjection): string {
 ...
 */
export function calendarTimedItemInstant(item: PlanTaskProjection | PlanMeetingProjection): string {
 ...
 */
export function compareCalendarTimedItems(
 ...
 */
export function visibleWeekEnd(startDate: string, windowEndDate: string): string {
 ...
 */
export function buildCalendarHolidaySegments(
    holidayRanges: readonly HolidayRangeProjection[],
    window: MeetingOccurrenceWindow,
 ...
 */
export function buildCalendarWindowProjection(
    tasks: readonly PlanTaskProjection[],
    meetings: readonly PlanMeetingProjection[],
    holidayRanges: readonly HolidayRangeProjection[],
    context: PlanEvaluationContext,
 ...
 */
export function agendaItemDate(item: AgendaItemProjection, context: PlanEvaluationContext): string {
 ...
 */
export function agendaItemRank(item: AgendaItemProjection): number {
    if (item.kind === 'holiday-range') {
        return 0;
    }
 ...
 */
export function agendaItemInstant(item: AgendaItemProjection): string {
    if (item.kind === 'meeting') {
        return item.occurrence.startInstant;
    }
 ...
 */
export function agendaItemIdentityKey(item: AgendaItemProjection): string {
 ...
 */
export function buildAgendaConflictWarnings(
    meetings: readonly PlanMeetingProjection[],
 ...
 */
export function buildAgendaProjection(
    calendar: CalendarWindowProjection,
    holidayRanges: readonly HolidayRangeProjection[],
    context: PlanEvaluationContext,
): AgendaProjection {
