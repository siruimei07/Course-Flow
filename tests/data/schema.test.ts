...
test('TEST-DATA-001/005: level 9 initializes Meeting, HolidayRange, and Task facts', async t => {
...
test('level 9 rejects representative constraint violations without changing bootstrap facts', t => {
    const dataSlotsRoot = createTempDataSlots(t);
    const store = initializeWorkspaceData(dataSlotsRoot, WORKSPACE_ID);
...
test('level 9 rejects a same-name index whose required properties drift', async t => {
    const dataSlotsRoot = createTempDataSlots(t);
    const store = initializeWorkspaceData(dataSlotsRoot, WORKSPACE_ID);
    await store.close();
...
test('Q-TIME-01/TEST-DATA-006: level 5 offsets migrate atomically and restart at zero', async (t) => {
    ...
    assert.equal(continued.store.status().revision, '9');
...
test('ADR-04/TEST-DATA-006: level 4 migrates occurrences with a restartable safety boundary', async (t) => {
    ...
    assert.equal(continued.store.readProtectionWatermark(), '9');
    await continued.store.close();
});
...
test('TEST-DATA-002/006: level 2 Term facts migrate to level 9 without identity loss', async t => {
    const dataSlotsRoot = createTempDataSlots(t);
    createLevel2WorkspaceWithTerm(dataSlotsRoot);
    ...
    assert.equal(opened.store.readPendingFollowUps().length, 1);
    assert.equal(opened.store.readProtectionWatermark(), '8');
    await opened.store.close();
...
test('A-COURSE-007/TEST-DATA-002/006: level 3 ranges and identities migrate to level 9', async t => {
    const dataSlotsRoot = createTempDataSlots(t);
    createLevel3WorkspaceWithCourse(dataSlotsRoot);
