...
- 八条核心 MVP-A 流程：首次启动、首次设置、今日首页、课程课表、任务、周日历、数据保护和学期切换。
...
- 待完成、已完成、已跳过和删除是不同语义。
- 大任务完成时显示 100%；恢复完成状态时还原完成前的自报进度。
- 重复任务每个实例拥有独立状态。
...
- 本地保存成功后，若配置了云盘目录，再异步更新带格式版本和完整性信息的快照。
- 备份失败不回滚本地成功修改，并保留上一个有效快照。
...
1. 应用检查默认活动数据目录和数据完整性。
2. 显示欢迎页，说明无需账户、核心功能离线可用、正式数据保存在本机。
3. 用户选择“开始新的本地数据”或“从备份恢复”。
4. 选择新数据时，应用初始化空白活动数据并进入 UF-A-02。
5. 选择恢复时，用户选择备份目录与快照。
6. 应用验证格式版本、完整性、可读性及数据范围。
7. 应用展示快照时间、学期、课程和事项范围的恢复预览。
8. 用户明确确认后执行可恢复导入；应用不自动扫描云盘、不自动选最新副本、不合并两个副本。
9. 恢复成功后，应用按恢复数据完整度进入“今天”或 UF-A-02。

**失败与恢复**：

- 快照损坏、不兼容、不可读或导入失败时，停留在选择页并解释原因。
- 原本地数据保持不变；用户可重选快照、取消或开始新数据。
...
1. 用户创建当前学期，填写名称、开始日、结束日和默认时区。
2. 用户可添加一个或多个命名假期；这一步不阻塞首次设置。
3. 用户创建至少一门课程，填写代码、名称、教学日期范围及可选 section、教授、颜色和学分。
4. 用户选择添加课节或添加任务。
5. 课节路径中，用户选择 LEC/TUT/PRA、星期、时间、地址和范围；教授信息从课程资料引用。
6. 任务路径中，用户选择课程、标题、小/大任务、纯日期/精确时间/TBA，并可配置每周重复。
7. 每个步骤本地保存成功后，清单立即更新；保存失败保留输入且不推进清单。
8. 达到最低条件后，应用标记“已可开始使用”，默认进入“今天”。
...
4. 首页分别选择下一个小任务和下一个大任务；TBA 任务不参与选择。
5. 用户可直接把当前任务实例标为完成或跳过。
6. 本地保存成功后，所有视图同步更新并短暂提供撤销。
7. 用户打开详情以编辑任务内容、重复规则或大任务进度。
...
3. 用户添加一条或多条课节规则。
4. 每条规则要求类型、星期、开始/结束时间；地址可为 TBA。
5. 规则默认继承课程教学范围；用户可设置位于课程范围内的更短范围。
6. 应用验证时间与范围，并显示所有重叠警告。
7. 用户确认保存后，应用按统一公式生成课节实例。
8. 今天、本周、日历和摘要同时刷新。
...
5. 已配置云盘目录时，应用异步生成或更新快照。
6. 备份成功时更新最后成功时间。
7. 备份失败时保留上一有效快照并显示错误；本地修改保持成功。
...
1. 用户从设置选择“导入备份”。
2. 用户选择快照。
3. 应用验证格式、完整性和兼容性，并预览影响。
4. 用户确认替换现有本地数据；MVP 不提供自动合并。
5. 若当前数据可读写，应用先创建恢复前安全快照。
6. 应用在临时位置完成导入验证，再原子替换当前数据。
7. 成功后重新计算当前学期与首次设置状态。
8. 任一步失败时继续使用原本地数据。
...
1. 用户进入“设置 → 计划与日历”。
2. 用户开启“课节出席记录”。
3. 功能默认关闭；开启从当前学期时区的当天开始生效。
4. 启用日期包含当天；当天较早已经结束的课节可以进入未标记状态。早于启用日期的课节不会新增状态、待确认事项或统计义务。
5. 关闭功能时，已有出席记录保留，课表和任务继续正常工作。
6. 再次开启时从新的开启当天开始，不回溯关闭期间或此前日期。

因此实现和验收必须支持一个或多个有效记录周期，而不能用开关变化静默重写历史。

**手工记录**：

1. 处于有效记录周期的课节开始后，“今天”和课节详情显示“打卡出席”和“标记缺席”。
2. 用户选择后，本地保存该课节实例的自报状态。
3. 状态为未标记、已出席或缺席；用户可更正或恢复为未标记。
4. 未标记保持未知，不能自动按缺席处理。
...
```text
已标记出席率 = 已出席 ÷ (已出席 + 缺席)
记录覆盖率 = (已出席 + 缺席) ÷ 已结束且应上课的课节
```

页面同时显示已出席、缺席、未标记、已标记出席率和记录覆盖率。分母为零时结果保持未知并解释原因。
...
**完成状态**：启用日期以前的数据不变；启用期内的课节可手工标记；课程统计同时显示结果和覆盖范围。
...
| 云盘目录不可写 | 本地成功、备份失败 | 本地已改变；旧快照保留 | 重试或更换目录 |
| 备份损坏 | 验证失败，不进入替换 | 未改变 | 重选快照 |
| 恢复中断 | 恢复失败，继续原数据 | 原数据保持 | 重试或取消 |
...
8. 本地保存成功、备份失败时，任务状态保持成功且旧快照可用。
9. 从损坏快照恢复失败时，原本地数据仍可打开。
10. 学期结束后自动归档，用户查看历史并创建新学期，旧课程不自动复制。
11. 出席记录默认关闭；启用当天以前的课节完全不改变。
12. 启用后手工标记出席/缺席、恢复未知，并按课程计算出席率与覆盖率。
13. 关闭再开启出席记录时，关闭期间不回溯，既有记录不删除。
